...
var Commands;
(function (Commands) {
    Commands[Commands["PlayAnimations"] = 0] = "PlayAnimations";
    Commands[Commands["UpdateComputedElements"] = 1] = "UpdateComputedElements";
    Commands[Commands["UpdateEntityAnimations"] = 2] = "UpdateEntityAnimations";
})(Commands || (Commands = {}));
class UpdateEntityAnimations {
    constructor() {
    ...
    run(_) {
        ...
        for (let e of system.getEntities()) {
            ...
            if (!isFound) {
                let entityAnimation = new Graphics.EntityAnimation([new Anim.StickmanIdle(), new Anim.StickmanRun()], e.entityUid);
        ...
        for (let computEl of foundComponents[0]) {
            for (let eA of entityAnimations) {
                if (computEl.ownerUid == eA.entityUid && eA.isChanged) {
                    computEl.displayElement = eA.displayElement;
                }
            }
            for (let p of foundComponents[1]) {
                if (computEl.ownerUid == p.ownerUid) {
                    let positionVector = p.position;
                    computEl.left = positionVector.x;
                    computEl.top = positionVector.y;
                }
            }
            for (let lD of foundComponents[2]) {
                if (computEl.ownerUid == lD.ownerUid) {
                    let isLookingRight = lD.isLookingRight;
                    if (isLookingRight) {
                        computEl.classes = ["state", "look-right"];
                    }
                    else {
                        computEl.classes = ["state", "look-left"];
                    }
                }
            }
        }
