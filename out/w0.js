        ...
        switch (msg.message) {
            case Utils.Messages.Done:
                ...
                for (let nC of newData.addedCommands) {
                    ...
                    workers[workerWithLessCommands[0]].messagePort.postMessage(new Utils.Message(Utils.Messages.AddedCommand, nC));
                }
                ...
                for (let pC of newData.changedProperties) {
                    ...
                    if (components[pC.componentType][pC.componentIndex].type == Comps.Components.ComputedElement) {
                        graphicDiff.changedComputedElements.push(new ECS.ComponentAndIndex(components[pC.componentType][pC.componentIndex], pC.componentIndex));
                    }
                    components[pC.componentType][pC.componentIndex][pC.property] = pC.value;
                }
                ...
                let workerInput = new Utils.WorkerInput(newData.changedProperties, newData.removedComponents, newData.addedComponents, input);
                for (let w of workers) {
                    w.messagePort.postMessage(new Utils.Message(Utils.Messages.Update, workerInput));
                }
                break;
        }
    });
...
function sendComputedElementsToRender() {
    ...
    for (let cAI of graphicDiff.changedComputedElements) {
        let cE = components[Comps.Components.ComputedElement][cAI.index];
        cE.isChanged = false;
