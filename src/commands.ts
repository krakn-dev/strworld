    ...
    run(system: ECS.System) {
        ...
        for (let fC of foundEntityTypeComponents[0]) {
            ...
            let foundDogComponents = system.find(
                [ECS.Get.One, [Comps.Components.Position, Comps.Components.TargetPosition, Comps.Components.Force], ECS.By.EntityId, dogUid])
            if (
                foundDogComponents[0].length == 0 ||
            ...
            let dogTargetPositionComponent = foundDogComponents[1][0].component as Comps.TargetPosition
            ...
            if (!isDogInPlayerRadius && Utils.randomNumber(20) == 10) {
                system.setProperty<Comps.TargetPosition, "x">
                    (foundDogComponents[1][0], "x", closestPlayerPositionComponent.x)
                system.setProperty<Comps.TargetPosition, "y">
                    (foundDogComponents[1][0], "y", closestPlayerPositionComponent.y)
                system.setProperty<Comps.TargetPosition, "isMovingToTargetPosition">
                    (foundDogComponents[1][0], "isMovingToTargetPosition", true)
            }
            ...
            if (
                ...
                system.setProperty<Comps.TargetPosition, "x">
                    (foundDogComponents[1][0], "x", newTargetLocation.x)
                system.setProperty<Comps.TargetPosition, "y">
                    (foundDogComponents[1][0], "y", newTargetLocation.y)
                system.setProperty<Comps.TargetPosition, "isMovingToTargetPosition">
                    (foundDogComponents[1][0], "isMovingToTargetPosition", true)
            }

            if (!dogTargetPositionComponent.isMovingToTargetPosition) continue

            let direction = new Utils.Vector2(0, 0)

            if ((dogTargetPositionComponent.y - dogPositionComponent.y) > -40) direction.y += 1
            if ((dogTargetPositionComponent.y - dogPositionComponent.y) < 40) direction.y -= 1
            if ((dogTargetPositionComponent.x - dogPositionComponent.x) > -40) direction.x += 1
            if ((dogTargetPositionComponent.x - dogPositionComponent.x) < 40) direction.x -= 1
            ...
            if (
                Math.abs(dogTargetPositionComponent.x - dogPositionComponent.x) < 10 &&
                Math.abs(dogTargetPositionComponent.y - dogPositionComponent.y) < 10
            ) {
                system.setProperty<Comps.TargetPosition, "isMovingToTargetPosition">(
                    foundDogComponents[1][0],
                    "isMovingToTargetPosition",
                    false
                )
            }
...
export class ApplyForce implements ECS.Command {
    ...
    run(system: ECS.System) {
        ...
        for (let fFC of foundForceComponents[0]) {
            ...
            if (forceComponent.x == 0 && forceComponent.y == 0) continue
